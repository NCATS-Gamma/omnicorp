...
object RoboCORD extends IOApp with LazyLogging {
  ...
  class Conf(arguments: Seq[String], logger: Logger) extends ScallopConf(arguments) {
    ...
    val outputPrefix: ScallopOption[String] = opt[String](
      descr =
        "Prefix for the filename (we will add '_from_<start index>_until_<end index>.txt' to the filename)",
    ...
    val currentChunk: ScallopOption[Int] =
      opt[Int](descr = "The current chunks (from 0 to totalChunks-1)", default = Some(0))
    val totalChunks: ScallopOption[Int] =
      opt[Int](descr = "The total number of chunks to process", default = Some(-1))
  ...
  def processEntries(conf: RoboCORD.Conf): Stream[IO, Unit] = {
    ...
    logger.info(
      s"Selected $articlesTotal articles for processing (from $startIndex until $endIndex, chunk $currentChunk out of $totalChunks)"
    )
    ...
    val preAnnotationStream: Stream[IO, (String, String)] = {
      metadataStream.map({
        case (entry, index) =>
          ...
          (s"$id\t$articleId\t${if (pmcid == "") "" else s"PMCID:$pmcid"}\t$withFullText", fullText)
      })
    }
