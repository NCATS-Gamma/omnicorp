...
def download_pubtator3(to_dir: str = ".",
    ...
    files = conn.mlsd()
    filenames_to_download = list(map(lambda x: x[0], filter(lambda x: xml_only and "XML" in x[0], files)))
...
@click.option("--to-dir", "to_dir", type=click.Path(dir_okay=True, file_okay=False), default=".",
...
def download(to_dir, pubtator3_ftp_url, ftp_username, ftp_password):
    ...
    logging.info(f"Loading PubTator3 from {pubtator3_ftp_url} into {to_dir}.")
    pass
